void main()
{
    ...
    int hat_power = cli_execute(`js Item.all().filter(x => toSlot(x) == Slot.get("hat")).filter(x => equippedAmount(x) == 1).map(x => getPower(x)).reduce((a, b) => a + b, 0)`);
    hat_power = hat_power * hat_multiplier;
    int pant_power = cli_execute(`js Item.all().filter(x => toSlot(x) == Slot.get("pants")).filter(x => equippedAmount(x) == 1).map(x => getPower(x)).reduce((a, b) => a + b, 0)`);
